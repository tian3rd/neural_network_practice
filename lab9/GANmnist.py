...
class _netG(nn.Module):
    def __init__(self, ngpu):
        ...
        self.main = nn.Sequential(
            ...
            nn.ConvTranspose2d(ngf * 2,     1, 2, 2, 2, bias=False),
            nn.Tanh()
...
f = open('resultsStandardGan.txt', 'w')
for epoch in range(opt.niter):
    for i, data in enumerate(dataloader, 0):
        ...
        noise.normal_(0, 1)
        fake = netG(#FILL IN - what is input to netG?)
        ...
        ###########################
        netG.zero_grad()
        label.data.fill_(#FILL IN - which labels are used for the generator?)  
        output = netD(fake)
        errG = criterion(output, label)
        #what do we do after calculating the loss and before taking a learning step?
        #FILL IN THIS LINE
