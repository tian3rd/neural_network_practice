...
def lossFun(inputs, targets, hprev):
  ...
  for t in range(len(inputs)):
    ...
    xs[t][inputs[t]] = 1
    hs[t] = np.tanh(np.dot(Wxh, xs[t]) + np.dot(Whh, hs[t-1]) + bh) #hidden state with tanh activation function
  ...
  for t in reversed(range(len(inputs))):
    ...
    dhnext = np.dot(Whh.T, dhraw) #backprop from hraw to the output of h from the previous timestep
